...
📖**The HTTP/REST API information** can be found [here](https://docs.onflow.org/http-api/).  The public Flow HTTP/REST access nodes are accessible at:
...
📖 **gRPC Access API URLs** can be found [here](https://docs.onflow.org/access-api/#flow-access-node-endpoints). `sdk.transport` must be specified if you wish to use the gRPC API.  The public Flow gRPC access nodes are accessible at:
...
```
Result output: [TransactionStatusObject](https://docs.onflow.org/fcl/reference/api/#gettransactionstatus)
...
After a transaction has been [built](./sdk-guidelines#build-transactions) and [signed](./sdk-guidelines#sign-transactions), it can be sent to the Flow blockchain where it will be executed. If sending was successful you can then [retrieve the transaction result](./sdk-guidelines#get-transactions).
nsactions).
