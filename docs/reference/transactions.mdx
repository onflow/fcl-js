...
Just like [scripts](../scripts), `fcl.mutate` is a [JavaScript Tagged Template Literal](https://styled-components.com/docs/advanced#tagged-template-literals) that we can pass Cadence code into, except it is used for transactions. Whereas, `query` is used for scripts.
...
If you run into trouble, get stuck or need some help, reach out to us on [Discord](https://discord.gg/k6cZ7QC), we are more than happy to help. Or, check out the [Quick Reference](../api) for a deeper dive.
...
This `fcl.mutate` will return a `transactionId`. We can pass the response directly to `fcl.tx` and then use the `onceSealed` method which resolves a promise when the transaction is sealed.
...
The `prepare` statements arguments directly map to the order of the authorizations in the `authorizations` array.
...
To learn more about `mutate`, check out the [API documentation](../api).