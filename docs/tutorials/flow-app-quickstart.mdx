...
- [Installation](./flow-app-quickstart#installation)
- [Configuration](./flow-app-quickstart#configuration)
- [Authentication](./flow-app-quickstart#authentication)
- [Querying the Blockchain](./flow-app-quickstart#querying-the-blockchain)
- [Initializing an Account](./flow-app-quickstart#initializing-an-account)
- [Mutating the Blockchain](./flow-app-quickstart#mutating-the-blockchain)
...
> Learn more about [configuring Discovery](../reference/discovery) or [setting configuration values](../reference/api#setting-configuration-values).
...
To authenticate a user, all an app has to do is call `fcl.logIn()`. Sign up and unauthenticate are all also as simple as `fcl.signUp()` and `fcl.unauthenticate()`.  Once authenticated, FCL sets an object called `fcl.currentUser` which exposes methods for watching changes in user data, signing transactions, and more. For more information on the `currentUser`, read more [here](../reference/api#current-user).
...
In the `args` section, we are simply passing it our user's account address from the user we set in state after authentication and giving it a type of `Address`.  For more possible types, [see this reference](../reference/api#ftype).
...
Now if you click the "Execute Transaction" button you'll see the statuses update next to "Transaction Status." When you see "4" that means it's sealed! Status code meanings [can be found here](../reference/api#transaction-statuses).
