...
- Optional: You application may optionally configure FCL with an application
    ...
    - Reconstructing the data the was signed to produce the
      ...
      const DomainTag = 
        rightPaddedHexBuffer(
            Buffer.from("FLOW-V0.0-user" + YourApplicationsDomainTag).toString("hex"), 
            32
        ).toString("hex")
...
Your application's backend must first generate a random message that you wish
for your user to sign, with their wallet, using the `user-signature` service.
Your application should retain this random message and associate it with the
`address` of the account the user is authenticating with for future reference.
...
Once the user's wallet has signed the message, your application with then verify
