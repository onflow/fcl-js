...
export async function resolveAccounts(ix) {
  if (isTransaction(ix)) {
    try {
      await debug("resolveAccounts")("T1", ix.authorizations, ix.accounts)
      await collectAccounts(ix, Object.values(ix.accounts))
      await debug("resolveAccounts")("T2", ix.authorizations, ix.accounts)
      await collectAccounts(ix, Object.values(ix.accounts))
      await debug("resolveAccounts")("T3", ix.authorizations, ix.accounts)
    } catch (error) {
