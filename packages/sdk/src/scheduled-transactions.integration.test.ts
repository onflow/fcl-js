...
const ACCESS_NODE = "http://localhost:8888"
  ...
  describe("Query Scheduled Transaction", () => {
    it(
      ...
      async () => {
        ...
        console.log("⏳ Waiting for scheduled transaction to execute (~10 seconds with buffer)...")
        // Wait for the scheduled transaction to execute (1 second delay + 9 second buffer)
        await new Promise(resolve => setTimeout(resolve, 10000))
        
        console.log("📤 Sending dummy transaction to trigger scheduled transaction execution...")
        // Send a dummy transaction to trigger block production and scheduled transaction execution
        const authz = createAuthzFunction(TEST_ADDRESS, 0, TEST_PRIVATE_KEY)
        ...
        console.log("   Dummy transaction sent:", dummyTx.transactionId)
        
        // Wait a bit for the dummy transaction to be processed
