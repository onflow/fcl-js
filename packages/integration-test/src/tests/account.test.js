...
describe("account test", () => {
  ...
  test("account test", async () => {
    function getAddressFromOutput(output) {
      // Get address from CLI output and trim the 0x part out of the result
      return (output.substring(output.indexOf("0x"), output.indexOf("0x") + 18)).substring(2)
    }
    ...
    const address1 = getAddressFromOutput(output)
    ...
    const address2 = getAddressFromOutput(output)
