...
// Adapter interface that abstracts the differences between UniversalProvider (web) and SignClient (React Native)
export interface WcClientAdapter {
  // Connect to a wallet and establish a session, it returns the connection URI and an approval promise
  connect(
    opts: WcConnectOpts
  ...
  // Get the current session (if any), for SignClient, this may return null and findValidSession should be used instead
...
export interface PlatformAdapter {
  // Open a deeplink URL, Browser: uses window.open or link element, React Native: uses Linking.openURL
  openDeeplink(url: string): void | Promise<void>

  // Check if the current platform is mobile, Browser: checks navigator.userAgent, React Native: always returns true
  isMobile(): boolean

  // Show a pairing modal with QR code (optional, web only), React Native implementations should not provide this
  showPairingModal?(
    uri: string,
    projectId: string,
    onClose: () => void
  ...
  // Validate a session (optional, used by React Native for stricter validation), Checks expiry, acknowledged status, and session existence
  validateSession?(
    client: WcClientAdapter,
    session: SessionTypes.Struct
  ): Promise<boolean>

  // Find a valid session (optional, used by React Native for session discovery), If sessionTopic is provided, validates that specific session, Otherwise finds the most recent valid session
  findValidSession?(
    client: WcClientAdapter,
    sessionTopic?: string
  ...
  // Check if deeplink should be opened for this request, Browser: checks if mobile and if user was already deeplinked, React Native: different logic for newly created sessions
  ...
  // Build a deeplink URL with appropriate parameters, Browser: simple URL construction, React Native: includes topic and redirect params
  buildDeeplinkUrl?(
    appLink: string,
    session: SessionTypes.Struct,
    redirectUri?: string
