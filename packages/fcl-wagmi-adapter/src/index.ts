...
import {
  createCrossVmConnector,
  createCrossVmUser,
  createProvider,
  CrossVmConnector,
} from "@onflow/fcl-ethereum-provider"

type FclWagmiAdapterParams = Parameters<typeof createProvider>[0] & {
  crossVmUser: ReturnType<typeof createCrossVmUser>
}
...
export function fclWagmiAdapter(params: FclWagmiAdapterParams) {
  ...
  return createConnector<Provider, Properties>(config => {
    async function _disconnect() {
      ...
      connect = connectInfo => wagmiConnector.onConnect(connectInfo)
    ...
    const wagmiConnector: ReturnType<CreateConnectorFn<Provider, Properties>> =
      {
        id: id,
        name: name,
    ...
    // Bind the a cross-vm connector
    const crossVmConnectorFn = createCrossVmConnector<{}>(crossVmConfig => {
      const crossVmConnector: CrossVmConnector<{}> = {
        uid: `${config.storage?.key}-cross-vm-connector`,
      ...
      // You can use the callbacks from `config` if needed:
      // e.g. config.onConnected(session) or config.onDisconnected()

      return crossVmConnector
    })

    // Register the cross-vm connector (this will bind the two connectors together)
    params.crossVmUser.registerConnector(crossVmConnectorFn)
