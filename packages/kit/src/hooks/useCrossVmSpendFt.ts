...
export interface UseCrossVmSpendFtMutateArgs {
...
export const getCrossVmSpendFtTransaction = (chainId: string) => {
...
}: UseCrossVmSpendFtArgs = {}): UseCrossVmSpendFtResult {
  const chainId = useFlowChainId()
  const cadenceTx = chainId.data
    ? getCrossVmSpendFtTransaction(chainId.data)
    : null
