...
export async function generateTemplateAuditId({audit}) {
  invariant(
    audit != undefined,
    "generateTemplateAuditId({ audit }) -- audit must be defined"
  )
  invariant(
    typeof audit === "object",
    "generateTemplateAuditId({ audit }) -- audit must be an object"
  )
  invariant(
    typeof audit.f_type === "InteractionTemplateAudit",
    "generateTemplateAuditId({ audit }) -- audit object must be an InteractionTemplate"
  )
  ...
  switch (audit.f_version) {
    ...
    default:
      throw new Error(
        "generateTemplateAuditId Error: Unsupported audit version"
      )
  }
}
